//{ Driver Code Starts
#include <bits/stdc++.h>
using namespace std;

struct Node {
    int data;
    struct Node* left;
    struct Node* right;

    Node(int x) {
        data = x;
        left = right = NULL;
    }
};

void sort_and_print(vector<int>& v) {
    sort(v.begin(), v.end());
    for (int i = 0; i < v.size(); i++)
        cout << v[i] << " ";
    v.clear();
}

void printLevelOrder(struct Node* root) {
    vector<int> v;
    queue<Node*> q;

    q.push(root);

    Node* next_row = NULL;

    while (!q.empty()) {
        Node* n = q.front();
        q.pop();

        if (n == next_row) {
            sort_and_print(v);
            next_row = NULL;
        }

        v.push_back(n->data);

        if (n->left) {
            q.push(n->left);
            if (next_row == NULL)
                next_row = n->left;
        }

        if (n->right) {
            q.push(n->right);
            if (next_row == NULL)
                next_row = n->right;
        }
    }
    sort_and_print(v);
}

Node* createTree(int parent[], int n);

/* Driver program to test above function*/


// } Driver Code Ends
/* node structure  used in the program

struct Node
{
    int data;
    struct Node* left;
    struct Node* right;

    Node(int x){
        data = x;
        left = right = NULL;
    }
}; */

class Solution {
    unordered_map<int, vector<int>> umpp;
  public:
    // Function to construct binary tree from parent array.
    Node *createTree(vector<int> v) {
        int rootValue = -1;
        for(int i = 0; i < v.size(); i++){
            umpp[v[i]].push_back(i);
            if(-1 == v[i]){
                rootValue = i;
            }
        }
        unordered_map<int, Node *> tree;
        queue<int> q;
        q.push(rootValue);
        while(!q.empty()){
            int val = q.front(); q.pop();
            if(tree.find(val) == tree.end())
                tree[val] =  new Node(val);
            
            if(umpp[val].size() == 2) {
                tree[umpp[val][0]] = new Node(umpp[val][0]);
                tree[umpp[val][1]] = new Node(umpp[val][1]);
                tree[val]->left = tree[umpp[val][0]];
                tree[val]->right = tree[umpp[val][1]];
                q.push(umpp[val][0]);
                q.push(umpp[val][1]);
            } 
            else if(umpp[val].size() == 1) {
                tree[umpp[val][0]] = new Node(umpp[val][0]);
                tree[val]->left = tree[umpp[val][0]];
                q.push(umpp[val][0]);
            }
        }
        return tree[rootValue];
    }
};

//{ Driver Code Starts.

int main() {
    int t;
    cin >> t;
    while (t--) {
        int n;
        cin >> n;
        vector<int> a;
        for (int i = 0; i < n; i++) {
            int x;
            cin >> x;
            a.push_back(x);
        }

        Solution ob;

        Node* res = ob.createTree(a);

        printLevelOrder(res);
        cout << endl;
    }

    return 0;
}

// } Driver Code Ends